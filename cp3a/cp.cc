...
constexpr double4_t double4_t_init {0,0,0,0};
...
void correlate(int ny, int nx, const float *data, float *result) {
    ...
    int ncd = nc*blocks;
    vector<double4_t> X(ncd * na, double4_t_init);
    ...
    for (int ib=0; ib<nc; ib++) {
        for (int jb=ib; jb<nc; jb++) {
            ...
            double4_t temp_array[blocks][blocks];
            for (int i = 0; i<blocks; i++) {
                for(int j = 0; j<blocks; j++) {
                    temp_array[i][j] = double4_t_init;
                }
            }

            
            for (int ka = 0; ka<na; ka++) {
                double4_t x0 = X[(ib*blocks + 0)*na + ka];
                double4_t x1 = X[(ib*blocks + 1)*na + ka];
                double4_t x2 = X[(ib*blocks + 2)*na + ka];
                double4_t x3 = X[(ib*blocks + 3)*na + ka];
                double4_t x4 = X[(ib*blocks + 4)*na + ka];
                double4_t x5 = X[(ib*blocks + 5)*na + ka];
                double4_t x6 = X[(ib*blocks + 6)*na + ka];

                double4_t y0 = X[(jb*blocks + 0)*na + ka];
                double4_t y1 = X[(jb*blocks + 1)*na + ka];
                double4_t y2 = X[(jb*blocks + 2)*na + ka];
                double4_t y3 = X[(jb*blocks + 3)*na + ka];
                double4_t y4 = X[(jb*blocks + 4)*na + ka];
                double4_t y5 = X[(jb*blocks + 5)*na + ka];
                double4_t y6 = X[(jb*blocks + 6)*na + ka];
            ...
            //             int iy = jb*blocks + y;
            //             if (iy >=  ny) continue;
            //             double4_t x0 = X[ix*na + ka];
            //             double4_t y0 = X[iy*na + ka];
